...
let
  ...
  projection = pkgs.runCommand "attune-swe-explore-population-projection-v1.parquet" {} ''
    ${projector} ${benchmark} ${multilingual} ${pro} "$TMPDIR/projection.json"
    ${parquetHelper} write-object "$out" < "$TMPDIR/projection.json"
  '';
  result = {
  version = 1;
