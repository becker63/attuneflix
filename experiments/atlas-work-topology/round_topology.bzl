...
def attune_round_artifacts_test(name = "round1_artifacts_test", role = "round1"):
    ...
    directory the script re-derives. Reads the control world boundary (the round
    documents are re-derived over the frozen world), not the whole control
    record."""
    native.sh_test(
        name = name,
        size = "medium",
...
def attune_round_reproducibility_test(name = "round2_reproducibility_test", role = "round2"):
    ...
    stays visible in the committed oracle document and the round report. Reads
    the control world boundary only."""
    native.sh_test(
        name = name,
        size = "medium",
